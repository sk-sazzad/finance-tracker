    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            MyApplicationTheme(darkTheme = isDarkTheme) {
                ...
                LaunchedEffect(pinEnabled, securityPin) {
                    if (pinEnabled && securityPin.isEmpty()) {
                        // Emergency: PIN enabled but no PIN set — disable PIN to prevent lockout
                ...
                DisposableEffect(lifecycleOwner) {
                    val observer = LifecycleEventObserver { _, event ->
                        if (event == Lifecycle.Event.ON_STOP) {
                            // Relock when app goes to background
                            if (pinEnabled) isUnlocked = false
                        }
                    }
                ...
                if (pinEnabled && securityPin.isNotEmpty() && !isUnlocked) {
                    PinLockScreen(
