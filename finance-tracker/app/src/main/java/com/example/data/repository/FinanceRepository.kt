...
class FinanceRepository(private val context: android.content.Context, private val financeDao: FinanceDao, private val debtDao: com.example.data.local.dao.DebtDao) {
    ...
    suspend fun payDebtEntryWithWallet(
        ...
        // 1. Update entry paid amount and status
        ...
        val wallet = financeDao.getWalletById(walletId)
        if (wallet != null) {
            financeDao.updateWallet(wallet.copy(balance = wallet.balance - payAmount))

            // 3. Log as EXPENSE transaction
            val person = debtDao.getAllDebtPersons().first().firstOrNull { p ->
                debtDao.getEntriesForPerson(p.id).first().any { it.id == entry.id }
            }
            val tx = TransactionEntity(
                amount = payAmount,
                type = "EXPENSE",
                ...
                walletId = walletId,
                date = System.currentTimeMillis(),
                note = "Debt paid: ${person?.personName ?: ""}",
                tags = "debt,payment",
                currency = wallet.currency,
                isRecurring = false
            )
