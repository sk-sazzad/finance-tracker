...
class FinanceRepository(private val context: android.content.Context, private val financeDao: FinanceDao, private val debtDao: com.example.data.local.dao.DebtDao) {
    ...
    suspend fun populateDefaultsIfEmpty() {
        ...
        // Populate Wallets
        val existingWallets = financeDao.getAllWallets().first()
        if (existingWallets.isEmpty()) {
            val defaultWallets = listOf(
                WalletEntity(name = "Cash", icon = "💵", color = "#00D4AA", balance = 0.0, currency = "BDT", includeInTotal = true),
                WalletEntity(name = "bKash", icon = "📱", color = "#E2125B", balance = 0.0, currency = "BDT", includeInTotal = true),
                WalletEntity(name = "Bank", icon = "🏦", color = "#3B82F6", balance = 0.0, currency = "BDT", includeInTotal = true),
                WalletEntity(name = "Nagad", icon = "🟠", color = "#F74B26", balance = 0.0, currency = "BDT", includeInTotal = true)
            )
    ...
    suspend fun getWalletCurrency(walletId: Long): String {
        return financeDao.getWalletById(walletId)?.currency ?: "BDT"
    }
    ...
    suspend fun deleteRecurringTransaction(rec: RecurringTransactionEntity) = financeDao.deleteRecurringTransaction(rec)
    ...
    suspend fun updateDebtPerson(person: DebtPersonEntity) = debtDao.updateDebtPerson(person)
    ...
    suspend fun addDebtEntry(entry: DebtEntryEntity, personType: String) {
        debtDao.insertDebtEntry(entry)

        val walletId = entry.walletId ?: return
        val wallet = financeDao.getWalletById(walletId) ?: return

        val isExpense = personType == "LENT" // giving money = expense
    ...
    suspend fun payDebtEntryWithWallet(
        entry: DebtEntryEntity,
        payAmount: Double,
        walletId: Long,
        personType: String
    ) {
        val updatedPaid = (entry.paidAmount + payAmount).coerceAtMost(entry.amount)
        val status = if (updatedPaid >= entry.amount) "PAID" else "PARTIALLY_PAID"
        debtDao.updateDebtEntry(entry.copy(paidAmount = updatedPaid, status = status))

        val wallet = financeDao.getWalletById(walletId) ?: return
        val isIncome = personType == "LENT" // receiving back money I lent = income
        val newBalance = if (isIncome) wallet.balance + payAmount else wallet.balance - payAmount
        financeDao.updateWallet(wallet.copy(balance = newBalance))
        ...
        val tx = TransactionEntity(
            ...
            walletId = walletId,
            date = System.currentTimeMillis(),
            note = "Debt ${if (isIncome) "received" else "paid"}",
            tags = "debt,payment",
            currency = wallet.currency,
            isRecurring = false
        )
