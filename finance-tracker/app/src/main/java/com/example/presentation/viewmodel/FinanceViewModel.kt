...
class FinanceViewModel(
    ...
    val debtPersons = repository.allDebtPersons.stateIn(viewModelScope, SharingStarted.Lazily, emptyList())
    val debtEntries = repository.allDebtEntries.stateIn(viewModelScope, SharingStarted.Lazily, emptyList())
    ...
    fun setCurrentMonth(month: Int) { _currentMonth.value = month }
    fun setCurrentYear(year: Int) { _currentYear.value = year }
    ...
    private suspend fun processRecurringTransactions() {
        ...
        for (rec in active.filter { it.isActive }) {
            if (today >= rec.nextDueDate) {
                // Time to process!
                val currency = repository.getWalletCurrency(rec.walletId)
                ...
                val appPrefs = com.example.data.local.preferences.UserPreferencesManager(getApplication())
                if (appPrefs.notificationsEnabled.first()) {
                    com.example.notifications.NotificationHelper.sendRecurringProcessedNotification(getApplication(), rec.amount, rec.note)
                }
    ...
    fun toggleRecurringActive(rec: RecurringTransactionEntity) {
        viewModelScope.launch {
            repository.updateRecurringTransaction(rec.copy(isActive = !rec.isActive))
        }
    }
    ...
    fun addTransaction(
        ...
        viewModelScope.launch {
            ...
            // If recurring, also create a RecurringTransactionEntity
            if (isRecurring) {
                val nextDueDate = when (frequency) {
                    "DAILY" -> date + 86400000L
                    "WEEKLY" -> date + (7 * 86400000L)
                    "YEARLY" -> {
    ...
    fun getDepositsForGoal(goalId: Long): Flow<List<SavingsDepositEntity>> =
        repository.getDepositsForGoal(goalId)

    fun addSavingsDeposit(goalId: Long, amount: Double, walletId: Long?, note: String) {
        viewModelScope.launch {
            repository.addSavingsDeposit(goalId, amount, walletId, note)
        }
    }

    // --- DEBTS (NEW SYSTEM) ---
    fun addDebtPerson(name: String, phone: String, type: String) {
        viewModelScope.launch {
            val person = DebtPersonEntity(
                personName = name,
                phoneNumber = phone,
                type = type
            )
            repository.addDebtPerson(person)
        }
    }

    fun deleteDebtPerson(person: DebtPersonEntity) {
        viewModelScope.launch {
            repository.deleteDebtPerson(person)
        }
    }

    fun updateDebtPerson(person: DebtPersonEntity) {
        viewModelScope.launch {
            repository.updateDebtPerson(person)
        }
    }

    fun addDebtEntry(personId: Long, amount: Double, reason: String, date: Long, walletId: Long?, personType: String) {
        viewModelScope.launch {
            val entry = DebtEntryEntity(
                personId = personId,
                amount = amount,
                reason = reason,
                date = date,
            ...
            repository.addDebtEntry(entry, personType)
        }
    }

    fun updateDebtEntry(entry: DebtEntryEntity) {
        viewModelScope.launch {
            repository.updateDebtEntry(entry)
        }
    }

    fun deleteDebtEntry(entry: DebtEntryEntity) {
        viewModelScope.launch {
            repository.deleteDebtEntry(entry)
        }
    }

    fun payEntryAmount(entry: DebtEntryEntity, amount: Double) {
        viewModelScope.launch {
            repository.payDebtEntry(entry, amount)
        }
    }

    fun payEntryAmountWithWallet(entry: DebtEntryEntity, amount: Double, walletId: Long, personType: String) {
        viewModelScope.launch {
            repository.payDebtEntryWithWallet(entry, amount, walletId, personType)
        }
    }

    fun payOverallAmount(personId: Long, amount: Double) {
        viewModelScope.launch {
            repository.payDebtOverall(personId, amount)
        }
    }

    fun payDebtOverallWithWallet(person: com.example.data.local.entities.DebtPersonEntity, amount: Double, walletId: Long, markSettled: Boolean) {
        viewModelScope.launch {
            repository.payDebtOverallWithWallet(person, amount, walletId, markSettled)
        }
    }
    ...
    fun exportToCSV(context: Context): Uri? {
        ...
        try {
            ...
            writer.println("ID,Amount,Type,CategoryId,CategoryName,WalletId,Note,Tags,Date,Currency")
            for (tx in transactions.value) {
                val formattedDate = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(Date(tx.date))
                val categoryName = when (tx.categoryId) {
                    -2L -> "Savings Deposit"
                    -3L -> "Debt Payment"
                    -4L -> "Debt Entry"
                    else -> categories.value.find { it.id == tx.categoryId }?.name ?: "Unknown"
                }

                writer.println("${tx.id},${tx.amount},${tx.type},${tx.categoryId},\"$categoryName\",${tx.walletId},\"${tx.note.replace("\"", "\"\"")}\",\"${tx.tags}\",$formattedDate,${tx.currency}")
            }
            writer.flush()
            writer.close()
    ...
    fun getFinancialHealthScore(): StateFlow<Int> {
        return combine(transactions, wallets) { txList, walletList ->
            val now = Calendar.getInstance()
            ...
            score += 10 // Assuming 10 for now since we don't have due dates on DebtPersonEntity yet
    ...
    fun setUpdateInfo(info: UpdateChecker.UpdateInfo) {
        _updateInfo.value = info
    }
    ...
    fun markUpdateAsSeen(context: Context, buildTime: String) {
        viewModelScope.launch {
            UpdateChecker.markUpdateSeen(context, buildTime)
        }
    }
}
